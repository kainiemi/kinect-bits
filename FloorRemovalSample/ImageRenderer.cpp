...
HRESULT ImageRenderer::EnsureResources()
{
    ...
    if (NULL == m_pRenderTarget)
    {
        ...
        m_pRenderTarget->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::Red, 0.5f), &m_pRedBrush);
...
/// Draws bounding rect on specified coordinates
/// </summary>
/// <param name="pImage">image data in RGBX format</param>
/// <param name="cbImage">size of image data in bytes</param>
/// <param name="
/// <returns>indicates success or failure</returns>
HRESULT ImageRenderer::Draw(BYTE* pImage, unsigned long cbImage, int bbLeft, int bbTop, int bbRight, int bbBottom)
{
    ...
    D2D1_RECT_F box = D2D1::RectF(bbLeft, bbTop, bbRight, bbBottom);
    m_pRenderTarget->DrawRectangle(&box, m_pRedBrush);
