...
private:
    ...
    const int BackgroundPixelCountThreshold = 16;

    const int KernelSize = 8;

    const int HorizontalBlocks = cDepthWidth / KernelSize;
    const int VerticalBlocks = cDepthHeight / KernelSize;

    int                     m_collectImagesCount;
    std::vector<UINT16*>    m_depthFrames;
    UINT16*                 m_pDepthRef;
    UINT32*                 m_pDepthRef32;
    UINT16*                 m_pFloorRef;
    struct PixelBox
    {
        int tlX;
